...
cairo_ps('/work/csp/vt17420/test.ps',width=12,height = 16)
par(oma=c(2,4,4,2))
layout(matrix(c(1:20,20),ncol=3,nrow=7,byrow=T),
       heights = c(4,4,4,4,4,4,2.5))
...
for (y in 1:6){
  ...
  if (y==1){mtext('WNMI_atemp2m', side=3,font=2,line=1)}
  mtext(yr_list[y], side=2,line=3,font=2)
  PlotEquiMap(var=diff_atemp_tmin[y,,],
              ...
              filled.continents=F)
  map('world',lwd=0.5,add = T)
  if (y==1){mtext('WNMI_atemp2m - WNMI_tmin', side=3,font=2,line=1)}
  PlotEquiMap(var=diff_atemp_tnight[y,,],
              ...
              filled.continents=F)
  map('world',lwd=0.5,add = T)
  if (y==1){mtext('WNMI_atemp2m - WNMI_tnight', side=3,font=2,line=1)}
...
ColorBar(brks=seq(0,5,0.25),
         ...
         subsampleg = 1,
         col_inf = 'white',
         col_sup='#5F031E',vertical = F,draw_separators = T)
...
ColorBar(brks=seq(-2.25,2.25,0.5),
         ...
         col_inf = col1[1],
         subsampleg = 1,
         col_sup=col1[length(col1)],vertical = F,
         draw_separators = T)
dev.off()
