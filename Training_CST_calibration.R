...
for (v in 1:length(var_list)){
  ...
  # HWMI 
  file_out <- paste0(path_out,'ERA5_HWMI_',var_list[v],'_1993_2016_all.ps')
  cairo_ps(file_out,width=14,height = 14)
  maps <- Apply(data=list(dat=data[[2]],mask=mask1$data),
                target_dims=c('lon','lat'),
                function(dat,mask){dat*mask})[[1]]
  col1 <- colorRampPalette(brewer.pal(9,'YlOrRd'))(20)
  s2dv::PlotLayout('PlotEquiMap',c(1,2),var=maps,
                   lat=mask1$lat,lon=mask1$lon,
                   brks=seq(0,5,0.25),
                   cols=col1,
                   colNA='lightgrey',
                   col_sup='#5F031E', country.borders=T,subplot_titles_scale=0.8,
                   titles=as.character(years), bar_extra_margin =c(4,2,2,2),
  ...
  # NDAYSQ90 
  file_out2 <- paste0(path_out,'ERA5_NDAYSQ90_',var_list[v],'_1993_2016_all.ps')
  cairo_ps(file_out2,width=14,height = 14)
  maps <- Apply(data=list(dat=data[[1]],mask=mask1$data),
                target_dims=c('lon','lat'),
                function(dat,mask){dat*mask})[[1]]
  col2 <- colorRampPalette(brewer.pal(9,'YlGn'))(20)
  s2dv::PlotLayout('PlotEquiMap',c(1,2),var=maps,
                   lat=mask1$lat,lon=mask1$lon,
                   brks=seq(0,5,0.25)*10,
                   cols=col2,
                   colNA='lightgrey',
                   col_sup='#00441b', country.borders=T,subplot_titles_scale=0.8,
                   titles=as.character(years), bar_extra_margin =c(4,2,2,2),
                   bar_label_scale = 1.3,draw_separators = T,
                   subsampleg = 2,
                   filled.continents=F)
